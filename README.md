...
設定ファイルは `%APPDATA%\HEICtoJPG\config.json` です。古い `{"output_dir": null}` 形式も読み込めます。未知のキーは無視されます。表示言語も同じ設定ファイルに保存されます。
...
The config file is `%APPDATA%\HEICtoJPG\config.json`. The old `{"output_dir": null}` format still loads. Unknown keys are ignored. The display language is saved in the same config file.
